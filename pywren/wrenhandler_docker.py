...
JOBRUNNER_CONFIG_NAME = "jobrunner_{0}.config.json"
...
def generic_handler(event, context_dict, custom_handler_env=None):
    ...
    try:
        ...
        jobrunner_config_name = JOBRUNNER_CONFIG_NAME.format(pid)
        shutil.copy(jobrunner_path, conda_runtime_dir)
        shutil.copy(jobrunner_config_filename, conda_runtime_dir)
        with open(os.path.join(conda_runtime_dir, "entry.sh"), "w+") as entry_script:
            entry_script.write("/runtime/bin/python /runtime/jobrunner.py /runtime/{}\n".format(jobrunner_config_name))
       ...
       #                            stdout=subprocess.PIPE, preexec_fn=os.setsid)
        ...
        client.containers.run(client.images.list()[0].id, auto_remove=True, volumes={os.readlink(conda_runtime_dir): {'bind': '/runtime', 'mode': 'ro'}})
